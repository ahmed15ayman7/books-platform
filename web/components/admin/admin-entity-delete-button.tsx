...
export function AdminEntityDeleteButton({
  apiPath,
  entityTitle,
  redirectHref,
  confirmTitle,
          ...
          <div className="w-full max-w-md rounded-xl border border-[var(--admin-border-strong)] bg-[var(--admin-surface)] shadow-2xl">
            <div className="flex items-start justify-between gap-3 border-b border-[var(--admin-border)] px-5 py-4">
              <div className="flex items-start gap-3">
                ...
                <div>
                  <h2 className="text-lg font-bold text-[var(--admin-text)]">{confirmTitle}</h2>
                  <p className="mt-1 text-sm text-[var(--admin-text-muted)]">
                    سينقل {entityLabel} إلى سلة المحذوفات.
                  </p>
                </div>
              </div>
              <button
                ...
                disabled={isPending}
                className="rounded-lg p-1 text-[var(--admin-text-muted)] hover:bg-[var(--admin-hover)]"
                aria-label="إغلاق"
              >
                <X className="h-5 w-5" />
              </button>
